...
def print_pdf_bytes(pdf: bytes, printer: Optional[str] = None) -> None:
    if not pdf or pdf[:4] != b"%PDF":
        # Attempt to tolerate leading bytes (BOM). Try to find %PDF
...
def _print_pdf_windows(path: str) -> None:
    ...
    try:
        ...
        pass
    raise PrintError("Windows printing failed: install a PDF reader and set it as default app for PDF.")
    ...
    try:
        if system == "windows" and win32print is not None:
            ...
            for p in win32print.EnumPrinters(flags):
                # p tuple varies; name often at index 2
                name = p[2]
