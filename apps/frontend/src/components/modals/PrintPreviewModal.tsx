...
export const PrintPreviewModal: React.FC<PrintPreviewModalProps> = ({
  ...
  const handlePrint = async () => {
    if (printMethod === 'server') {
      ...
      try {
        // Use backend print-job API (idempotent)
        const idempotencyKey = `ticket-${ticket.ticketNo || ticket.id}`;
        const payload = {
          machineId,
          idempotencyKey,
          copies: 1,
          ...
          weighOutWeight: ticket.tareWeight,
          netWeight: ticket.netWeight,
          direction: ticket.weighOutTime ? 'OUT' : 'IN',
        } as const;

        const result = await apiService.createPrintJob(payload as any);
        if (result?.id) {
